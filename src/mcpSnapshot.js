...
const NAVIGATE_SCHEMA_PATH = fileURLToPath(
  new URL('../data/mcp-navigate-schema.json', import.meta.url)
);
...
async function preflightSchemas(onLog) {
  ...
  const checks = [
    ...
    { label: 'mcp navigate', path: NAVIGATE_SCHEMA_PATH }
  ];
...
function buildNavigatePrompt({ url, pageId, cachedPages, skipListPages } = {}) {
  const targetLabel =
    typeof pageId === 'number' && Number.isFinite(pageId)
      ? `page id ${pageId}`
      : `URL ${url}`;
    ...
    typeof pageId === 'number' && Number.isFinite(pageId)
      ...
      : normalizedCachedPages
        ...
        : skipList
          ...
          : '- Liste les pages avec list_pages; si une page correspond à l’URL, sélectionne la première dans l’ordre retourné; sinon navigue vers l’URL avec navigate_page.',
    `2) Vérifie que location.href correspond bien à la cible (${targetLabel}); si besoin navigue vers l’URL.`,
    '3) Réponds avec un JSON {"ok": true/false, "url": "<location.href>"} uniquement.',
    '',
    `URL: ${url}`,
    typeof pageId === 'number' && Number.isFinite(pageId) ? `PAGE_ID: ${pageId}` : '',
    normalizedCachedPages ? `CACHED_PAGES: ${JSON.stringify(normalizedCachedPages)}` : '',
...
async function runCodexNavigate({ url, model, mcp, onLog, onStage, signal }) {
  if (signal?.aborted) {
    throw createAbortError();
  }
  ...
  const outputFile = path.join(
    os.tmpdir(),
    `codex-mcp-navigate-${Date.now()}-${Math.random().toString(16).slice(2)}.json`
  );
  ...
  onStage?.('AI: preparing MCP navigate');
  onLog?.('Codex: preparing MCP navigate');
  ...
  const runOnce = async (env, args) =>
    new Promise((resolve, reject) => {
      ...
      onStage?.('AI: running MCP navigate');
      onLog?.('Codex: running MCP navigate');
      attachIgnoreEpipe(child.stdin, (err) => {
        onLog?.(`Codex: stdin error (${err?.code || 'unknown'}).`);
      });
      child.stdin.write(
  ...
  try {
    await runOnce(preferredEnv, buildArgs(mcp, NAVIGATE_SCHEMA_PATH));
  } catch (err) {
    maybeHandleMissingAuth({ onLog, stderr: err?.stderr || err?.message });
    if (model && looksLikeModelNotFound(err.stderr)) {
      onLog?.(`Codex: model ${JSON.stringify(model)} not found; retrying with default model`);
      ...
      if (canFallback && looksLikeMcpConnectError(err.stderr)) {
        ...
        fallbackConfig.autoConnect = true;
        try {
          await runOnce(preferredEnv, buildArgs(fallbackConfig, NAVIGATE_SCHEMA_PATH));
        } catch (fallbackErr) {
          throw decorateCodexError(fallbackErr);
        }
        ...
        onLog?.(`Codex: retrying with CODEX_HOME=${fallbackHome}`);
        try {
          await runOnce(buildCodexEnv({ codexHome: fallbackHome }), buildArgs(mcp, NAVIGATE_SCHEMA_PATH));
        } catch (fallbackErr) {
          throw decorateCodexError(fallbackErr);
        }
      } else {
  ...
  onStage?.('AI: parsing MCP navigate');
  onLog?.('Codex: parsing MCP navigate');
  const content = await fs.readFile(outputFile, 'utf-8');
  await fs.unlink(outputFile).catch(() => {});
  return JSON.parse(content);
}

export async function navigateMcpToUrl({ url, model, mcp, onLog, onStage, signal }) {
  if (
    mcp &&
    !mcp?.pageId &&
  ...
  return runCodexNavigate({ url, model, mcp, onLog, onStage, signal });
}
